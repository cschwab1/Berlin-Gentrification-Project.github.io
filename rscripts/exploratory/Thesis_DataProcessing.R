##########################################################################################
# ENST and GEOG BA Thesis 2020/2021 
# R doc
##########################################################################################
...
brw_processing <- function(x){
  ...
  y <- x %>%
    ...
    dplyr::select(gml_id, BRW, geometry, NUTZUNG) %>%
    filter(!st_is_empty(.))
  val <- y$BRW
...
brw2003comb <- rbind(brw2002, brw2003, brw2004)
brw2011comb <- rbind(brw2010, brw2011, brw2012)
...
v <- voronoi(as_Spatial(brw2003comb))
berlin.sf_test <- aggregate(as_Spatial(berlin.sf))
vberlin <- intersect(v, berlin.sf_test)
b02test <- rasterize(vberlin, berlin_template, "BRW")

brw2003comb <- as_Spatial(brw2003comb)
set.seed(5132015)
...
gs <- gstat(formula=BRW~1, locations=b02test, nmax=5, set=list(idp = 0))
nn <- interpolate(berlin_template, gs)
nnmsk <- mask(nn, b02test)
plot(nnmsk)
...
for (k in 1:5) {
  test <- brw2003comb[kf == k, ]
  train <- brw2003comb[kf != k, ]
  gscv <- gstat(formula=BRW~1, locations=train, nmax=5, set=list(idp = 0))
  p <- predict(gscv, test)$var1.pred
  rmsenn[k] <- RMSE(test$BRW, p)
}

gs <- gstat(formula=BRW~1, locations=vberlin)
idw <- interpolate(berlin_template, gs)
